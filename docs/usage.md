...
def pydsl_func():
	...
	# more pydsl code
```
Note how in addition to the parameters defined by the kernel, you also need to provide 3 extra parameters, which defines the grid index that you want to call the kernel with (x, y, z). A more detailed example of how to use the interop feature can be found in [examples/triton_interop.py](/examples/triton_interop.py).

For calling a triton function with a constexpr parameter, a `Number` object that has an explicit name must be passed in, i.e.
```python
@triton.jit
