...
def matmul(
    ...
    if not x.element_type == y.element_type == init.element_type:
        raise Exception(
    ...
    if not issubclass(x.element_type, (Float, Int)):
        raise TypeError(
            ...
            f"linalg.matmul operations. Only Float and Int "
            f"are supported"
        )
    ...
    if not (len(x.shape) == len(y.shape) == len(init.shape) == 2):
        raise Exception("operands and init need to have rank of 2")
    if not (
    ...
    if any([d == DYNAMIC for d in x.shape + y.shape + init.shape]):
        raise Exception(
            "support for DYNAMIC demensions has not been implemented"
        )
