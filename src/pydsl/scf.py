...
class SCFOp(ABC, Generic[ASTN, MLIRN]):
    """
    Base class for lowering PyDSL AST control-flow nodes into MLIR SCF ops.

    Specifically:
      • `for i in scf.range(...)` → `scf.ForOp`
      • `if ...:` → `scf.IfOp`

    Usage
    -----
    This code is used to lower PyDSL code containing `scf.range(...)` or `if`
    statements. When the AST is visited, the lowering is triggered through
    calls like `pydsl.scf.ForOp(...)` or `pydsl.scf.IfOp(...)`. These are not
    the raw MLIR ops from `mlir.dialects.scf`, but overloaded subclasses
    defined here (`ForOp` and `IfOp`) that implement the translation logic.

    Challenge
    ---------
    MLIR SCF ops (e.g., IfOp, ForOp) require the types of their yielded
    results to be fixed at construction. However, those types are only
    known *after* visiting the AST body of the op
    (e.g., the `if` branches or loop body). This creates a chicken-and-egg
    problem: we need an op to host the lowered body while visiting it,
    but once built, an op's type signature cannot be mutated.

    Solution
    --------
    Construction is unified through `SCFOp.__new__`, which handles:
      1. construct a temporary "dummy" SCF op with no result types.
         The dummy op's blocks temporarily host the lowered body while we
         visit it.
      2. Visit the AST body to discover which variables are yielded and what
         their types are.
      2. Construct the *real* SCF op with the correct types and initial values
         (since op types are immutable and cannot be retrofitted).
      3. Copying the lowered blocks from the dummy op into the real one, wiring
         up yields and block arguments.

    Subclasses (`IfOp`, `ForOp`) implement the op-specific details like
    how to construct the op and how to verify type consistency.

    Subclasses implement methods:
    - build_op(op_type, op_args, dummy)
        Construct the SCF op itself (dummy or real) with the given types
        and initial values.
    - build_regions(op)
        Lower the AST body into the op's regions and return the new
        variable bindings from each region.
    - verify_types(var, types, node):
        Check consistency of a variable's type across prior scope and
        regions; return the resolved type or a Poison value on error.
    - excluded_vars() -> tuple
        Return variable names that should not be treated as yielded
        results (e.g., loop induction variables).
    """
