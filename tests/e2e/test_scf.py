...
from pydsl.type import Bool, F16, Index, SInt32, Tuple, UInt16, UInt32
...
if __name__ == "__main__":
    ...
    run(test_const_if)
    run(test_fold)
    run(test_if_local_type_match)
    run(test_if_local_type_mismatch_poison_not_read)
    run(test_if_local_type_mismatch_poison_read)
    run(test_if_else_local_type_mismatch_poison_read)
    run(test_if_local2)
    run(test_if_else_local)
    run(test_if_else_local_nested)
    run(test_range_yield)
